...
class Connect4World:
    ...
    def __init__(self, rows=6, cols=7, player1=None, player2=None):
        ...
        self.canvas = Canvas(
            self.window, width=self.width, height=self.height, bg="#ffffcc"
        )
        ...
        for i in range(self.cols):
            b = Button(
                ...
                command=lambda col=i: self.next_action(col),
            )
            b.place(x=20 + 80 * i, y=60, anchor=NW, width=80, height=30)
            self.buttons.append(b)

        self.l_turn = Label(
            self.window, text="Current player: ", font=("Helvetica", 16), bg="#ffffcc"
        )
        self.l_turn.place(x=20, y=20)

        self.l_player = Label(
            self.window, text="", font=("Helvetica", 16), bg="#ffffcc", fg="red"
        )
...
if __name__ == "__main__":
    ...
    minimax_player = MinimaxPlayer(5)
    alphabeta_player = AlphabetaPlayer(5, eval_board)
    world = Connect4World(6, 7, player1=minimax_player, player2=minimax_player)
